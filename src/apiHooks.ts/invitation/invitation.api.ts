...
export const useCreateInvitation = () => {
  ...
  return useMutation({
    ...
    onSuccess: (result) => {
      ...
      toast.success("Invitation created", "The invitation has been created.");
    },
    retry: false,
    onError: (error: any) => {
      const message = (error as Error)?.message || "Invitation creation failed";
...
export const useGetInvitations = () => {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await request<GetInvitationsResponse>(
        ENDPOINTS.GET_ALL_INVITATIONS,
        "GET",
      );
      return res.data;
    },
...
export const useGetInvitationByToken = (
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const res = await request<GetInvitationByTokenResponse>(
        ENDPOINTS.GET_INVITATION_BY_TOKEN(token),
        "GET",
      );
      return res.data;
    },
...
export const useAcceptInvitation = (callbacks?: { onSuccess?: () => void }) => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success("Invitation accepted", "The invitation has been accepted.");
      callbacks?.onSuccess?.();
    },
    retry: false,
    onError: (error: any) => {
...
export const useDeclineInvitation = (callbacks?: {
  onSuccess?: () => void;
}) => {
  const queryClient = useQueryClient();
  return useMutation({
    ...
    retry: false,
    onError: (error: any) => {
      const message = (error as Error)?.message || "Invitation decline failed";
