    ...
    <div onClick={onClick} key={id} className="flex flex-col h-[100px]">
      <div key={org.id} className="flex items-start gap-3 mb-2">
        ...
        <div className="flex-shrink-0 flex ">
          ...
          {org.permissionNames?.includes("og:delete::organization") && org.memberships?.some((membership) => membership.role === "OWNER" || membership.role === "ADMIN") && (
            <Button
              variant="basic"
      ...
      <div className={`mt-auto rounded-lg transition-all  bg-primary/10`}>
        <div className="flex items-center justify-between px-2 py-1 ">
          <span className="text-[14px] font-medium text-primary">
            {org?._count?.memberships ? org?._count.memberships : "0"} member
            {org?._count?.memberships.length > 1 ? "" : "s"}
          </span>
