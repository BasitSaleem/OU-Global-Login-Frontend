  ...
  return (
    <>
      <div className="text-center md:text-left mb-9">
        <h1 className="text-heading-1 font-bold pt-8 pb-2">{loading ? <Dots text="Loading Billing" position="left" /> : "Billing"}</h1>
        {loading ? <p className="text-body-small">
          ...
          loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 md:gap-0 py-6 px-4 md:px-9">
              ...
              <div className="flex flex-col items-center md:items-start text-center md:text-left">
                ...
                <p className="text-body-small">
                  {subscriptions !== undefined && (subscriptions?.[0]?.status === "TRIAL" ||
                    subscriptions?.[0]?.status === "CANCELLED")
                    ? "--"
