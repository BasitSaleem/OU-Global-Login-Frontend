...
export default function UserProfileLayout({ children }: UserProfileLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-white flex font-inter">
      {/* Profile Sidebar */}
      <div className={`${sidebarCollapsed ? 'w-[88px]' : 'w-[280px]'} transition-all duration-300 bg-white border-r border-gray-200 flex flex-col`}>
        {/* Logo */}
            ...
            {profileNavItems.map((item) => (
              <a
                ...
                className={`
                  ...
                  ${!sidebarCollapsed && !item.isActive ? 'gap-3' : ''}
                `}
